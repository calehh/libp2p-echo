...
func init() {
	flag.IntVar(&rounds, "round", 1, "set echo rounds")
}
...
func makeBasicHost(listenPort int, insecure bool, randseed int64) (host.Host, error) {
	...
	opts := []libp2p.Option{
		libp2p.ListenAddrStrings(fmt.Sprintf("/ip4/127.0.0.1/tcp/%d", listenPort),
			fmt.Sprintf("/ip4/127.0.0.1/udp/%d/quic", listenPort)),
...
func getHostAddress(ha host.Host) []string {
	...
	list := make([]string, 0)
	for _, a := range addr {
		list = append(list, a.Encapsulate(hostAddr).String())
	}
	return list
}
...
func runSender(ctx context.Context, ha host.Host, targetPeer string) {
	...
	log.Println(fmt.Sprintf("average spend %v", time.Now().Sub(start).Milliseconds()/int64(rounds)))
}
